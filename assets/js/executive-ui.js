...
var scenarioRiskEl = byId("ceoModalScenarioRisk");
      ...
      const EXEC_BRIEFING_PAGES = {
        ceo: "ceo-brief.html",
        cfo: "cfo-impact-analysis.html",
        cto: "cto-operations-deepdive.html"
      };

      function appendExecBriefingLink(role) {
        const modalBody = byId("execModalBody");
        if (!modalBody) return;
        const href = EXEC_BRIEFING_PAGES[role] || EXEC_BRIEFING_PAGES.ceo;
        if (modalBody.querySelector(".exec-open-briefing-wrap")) return;

        const wrap = document.createElement("div");
        wrap.className = "exec-open-briefing-wrap";
        wrap.style.marginTop = "18px";
        wrap.innerHTML = `
          <div class="exec-modal-actions">
            <a href="${href}" class="zp-report-btn primary" style="text-decoration:none;display:inline-flex;align-items:center;">
      ...
      function openExecModal(role, mode) {
        ...
        if (!modalBody.querySelector(".exec-detail-link-row") && detailHrefMap[role]) {
          const wrap = document.createElement("div");
          wrap.className = "exec-detail-link-row";
          wrap.style.marginTop = "18px";
